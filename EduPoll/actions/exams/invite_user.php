...
if(examIsUserInvited($exam_id, $user)) {
	$_SESSION['error_messages'][] = 'That user has already been invited.';
	http_response_code ( 400 );
	exit;
}

try {
	if(examInviteUser($exam_id, $user) == -1) {
		$_SESSION['error_messages'][] = 'Error inviting user to exam.';
		http_response_code ( 400 );
		exit;
	}
} catch (PDOException $e) {
	$_SESSION['error_messages'][] = 'Error inviting user to exam: ' . $e->getMessage();
	http_response_code ( 400 );
	exit;
}

$_SESSION['success_messages'][] = 'User successfully invited.';
